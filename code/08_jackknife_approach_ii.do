...
**# Generate Income Quartile Interaction Coefficients (without reform interactions)
forvalues t = 2/7 {
    forvalues q = 2/4 {
        gen inc_`t'_`q' = .
    }
...
**# Generate Reform Type Interaction Coefficients (two-way: lag × reform)
...
foreach r of local reforms {
    forvalues t = 2/7 {
        capture scalar coeff_ref = _b[1.lag_`t'#1.`r']
        if _rc == 0 {
...
**# Generate Three-Way Interaction Coefficients (lag × income × reform)
foreach r of local reforms {
    forvalues t = 2/7 {
        forvalues q = 2/4 {
            gen inc_`r'_`t'_`q' = .
...
foreach r of local reforms {
    forvalues t = 2/7 {
        forvalues q = 2/4 {
            capture scalar coeff_inc_ref = _b[1.lag_`t'#`q'.inc_q#1.`r']
            if _rc == 0 {
                replace inc_`r'_`t'_`q' = coeff_inc_ref
            }
            else {
                replace inc_`r'_`t'_`q' = 0
            }
        }
    }
}
...
*--- Average income interactions (two-way only)
forvalues q = 2/4 {
    egen avg_inc_`q' = rowmean( ///
        inc_2_`q' inc_3_`q' inc_4_`q' inc_5_`q' inc_6_`q' inc_7_`q')
}

*--- Average reform type interactions (two-way only)
foreach r of local reforms {
    egen avg_`r' = rowmean(`r'_2 `r'_3 `r'_4 `r'_5 `r'_6 `r'_7)
}

*--- Average three-way interactions (lag × income × reform)
foreach r of local reforms {
    forvalues q = 2/4 {
...
*--- Add three-way interaction effects (lag × income × reform)
foreach r of local reforms {
    forvalues q = 2/4 {
