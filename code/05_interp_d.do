...
WHY THIS MATTERS (Workflow Context):
  ...
  - District data has fewer gaps than tract panel (districts more stable over time)
  - Interpolation at district level → more accurate than interpolating aggregates
  - Allows us to handle district boundary changes and mergers more cleanly

  Why weight by enrollment instead of population?
  - School-age population varies within counties (some areas skew older)
  - Enrollment captures which districts actually serve children (relevant for policy)
...
KEY ASSUMPTIONS & SENSITIVE STEPS:
  ...
  4. Enrollment Weighting (Step 4-5):
     ...
     - Counties with missing enrollment → drops (cannot weight properly)
  ...
  6. Panel Structure:
     ...
     - Unbalanced in counties: not all counties have data for all years
     - balance.do (separate file) identifies counties with complete event windows
