...
WHAT THIS FILE DOES (Summary):
  ...
  • Assigns exactly ONE LEAID per tract (using max population)
...
WHY THIS MATTERS (Workflow Context):
  ...
  Problem: Multiple districts can serve the same tract.
...
Districts with exactly one tract: 4946 out of 12463 (39.69%)

*******************************************************************************/
 *Collapse into unique tract x district x district type combos
collapse (sum) alloc_pop, by(tract70 sdtc LEAID)
...
preserve
bysort tract70 sdtc alloc_pop: gen n_ties = _N
tab n_ties state_fips if n_ties > 1 
restore
